...
private:
    ...
    InstructionType Simulator::classifyInstructions(uint32_t);
...
public:
    Simulator();
    void loadProgram(const std::string &input);
    bool step();
    void run();
...
void Simulator::loadProgram(const std::string &input) {
    ...
    while (getline(ss, line)) {
        ...
        if (!(line_ss >> addrStr >> instStr)) {
            throw std::runtime_error("Error: Malformed instruction line - " + line);
        }
